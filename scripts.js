document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchWeatherByCoords = (lat, lon) => {
        fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${apiKey}&units=metric`)
            ...
            .then(data => {
                ...
                // Update the map marker if the map is initialized
    ...
    const updateTime = (timezoneOffset) => {
        ...
        const timeString = localTime.toUTCString().replace(/ GMT$/, ""); // Format the time string to remove 'GMT'
    ...
    // Load favorite places if on favorites.html
